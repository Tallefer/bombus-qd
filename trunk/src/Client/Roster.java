...
//#       private Command cmdConference=new Command(SR.MS_CONFERENCE, Command.SCREEN, 24);
//#       private Command cmdAdd=new Command(SR.MS_ADD_CONTACT, Command.SCREEN, 25);
//#       private Command cmdSearchUsers=new Command(SR.MS_USERS_SEARCH, Command.SCREEN, 47);
//#ifdef SERVICE_DISCOVERY      
//#       private Command cmdSrvDisco=new Command(SR.MS_DISCO, Command.SCREEN, 26);
//#endif                              
//#ifdef PRIVACY      
...
//#endif                  
//#       
//#     private Command cmdMyService=new Command(SR.MS_SERVICE, Command.SCREEN, 31);   //3       
//#       private Command cmdAlert=new Command(SR.MS_ALERT_PROFILE_CMD, Command.SCREEN, 32);
//#       private Command cmdArchive=new Command(SR.MS_ARCHIVE, Command.SCREEN, 33);
//#ifdef STATS       
//#       private Command cmdStats=new Command(SR.MS_STATS, Command.SCREEN, 34);  
//#endif                  
//#       private Command cmdClrAllChats=new Command(SR.MS_CLEAN_ALL_MESSAGES, Command.SCREEN, 35);
...
//#if SASL_XGOOGLETOKEN      
//#       private Command cmdGmail=new Command(SR.MS_CHECK_GOOGLE_MAIL, Command.SCREEN, 45);   //3       
//#endif       
//#       private Command cmdFM=new Command(SR.MS_FILE_MANAGER, Command.SCREEN, 46);   //3
//#endif      
    ...
    private Command cmdLatestNews=new Command("QD ".concat(SR.MS_QD_NEWS), Command.SCREEN, 82);
...
//#                 if (cs.vcard!=null) {
//#                     new VCardEdit(display, this, cs.vcard);
//#                     return;
//#                 }
...
//#          */ 
//#ifdef STATS        
//#            else if(c==cmdStats){  new StatsWindow(display, this);  }
//#endif                        
...
//#endif
   public void cmdActions() {
    ...
    public void run(){
//#ifdef POPUPS
        //if (midlet.BombusQD.cf.firstRun) setWobbler(1, (Contact) null, SR.MS_ENTER_SETTINGS);
//#endif
        setQuerySign(true);
	if (!doReconnect) {
            setProgress(25);
...
//#endif
	...
	setMyJid(new Jid(midlet.BombusQD.sd.account.getJid()));
	updateContact(midlet.BombusQD.sd.account.getNick(), myJid.getBareJid(), SR.MS_SELF_CONTACT, "self", false);
    }
...
//#endif
        ...
        // creating room
        if (grp==null) groups.addGroup(grp=new ConferenceGroup(roomJid, room) ); // we hasn't joined this room yet
    ...
    public final MucContact mucContact(String from){
        ...
        if (c==null) {
            c=new MucContact(from.substring(rp+1), from);//initContact if null
            addContact(c);
...
//#endif
   ...
   private void sortContacts(Contact sort){
        try {
            synchronized (hContacts) {
                int f, i;
                ...
                Group g = null;
                Contact c = null;
                if(sort!=null) g = sort.group;
                for (f = 1; f < size; f++) {
                     c = (Contact)hContacts.elementAt(f);
                     if(sort!=null && c.group!=g) continue;
                        left=(IconTextElement)hContacts.elementAt(f);
                        right=(IconTextElement)hContacts.elementAt(f-1);
                        if ( left.compare(right) >=0 ) continue;
                        i = f-1;
                        while (i>=0){
                           c = (Contact)hContacts.elementAt(i);
                           if(c.metaContact) break;
                           right=(IconTextElement)hContacts.elementAt(i);
                           if (right.compare(left) <0) break;
                           hContacts.setElementAt(right,i+1);
                           i--;
                        }
                        hContacts.setElementAt(left,i+1);
                }
            }
            reEnumRoster();
        } catch (Exception e) {
...
//#ifndef WMUC
    public void multicastConferencePresence(int myStatus, String myMessage, int myPriority) {
         //if (!midlet.BombusQD.cf.autoJoinConferences) return;
         //?
...
//#endif
    ...
    public void resolveNicknames(String transport){
        vCardQueue=null;
	    vCardQueue=new Vector(0);
        int size = hContacts.size();
        Contact k;
        synchronized (hContacts) {
            for(int i=0;i<size;i++){
                k =(Contact)hContacts.elementAt(i);
                    ...
                    if (id.equals("getros")){
                        ...
                        if(midlet.BombusQD.cf.isStatusFirst && firstStatus!=-1){
                          if (firstStatus==5) {
                            ...
                            sendPresence(firstStatus, null);  
                          }                            
                          midlet.BombusQD.cf.isStatusFirst=false;
                          firstStatus=-1;
                        }else{
                          if (midlet.BombusQD.cf.loginstatus==5) {
                            sendPresence(Presence.PRESENCE_INVISIBLE, null);    
                          } else {
...
//#                       if(midlet.BombusQD.cf.actCat!=null){
//#                        pub.append(SR.MS_ACTIVITY + ": " + locale.Activity.loadString(midlet.BombusQD.cf.actCat));                         
//#                       
//#                        pub.append("(");
//#                        if(midlet.BombusQD.cf.actDescr!=null){ 
//#                           pub.append(locale.Activity.loadString(midlet.BombusQD.cf.actDescr));
//#                        }
//#                         if(midlet.BombusQD.cf.actText!=null){
//#                           pub.append(" : "+locale.Activity.loadString(midlet.BombusQD.cf.actText));                         
//#                         }
//#                         pub.append(")\n");
//#                      }
//#                      
//#                      int index = Moods.getInstance().getMoodIngex(midlet.BombusQD.cf.moodName);
//#                      pub.append(SR.MS_USERMOOD + ": " + Moods.getInstance().getMoodLabel(index));
//#                        if(midlet.BombusQD.cf.moodText.length()>1){
//#                           pub.append("("+midlet.BombusQD.cf.moodText+")");                         
//#                        }
//#                      
//#                      new AlertBox("Send form", SR.MS_UP_MOODS + "\n\n" + pub.toString(), display, this) {
//#                        public void yes() {
//#                                try {
//#                                   Thread.sleep(3000);
//#                                 } catch( Exception e ) {}
//#                           publishEvent.publishActivity(midlet.BombusQD.cf.actCat,midlet.BombusQD.cf.actDescr,midlet.BombusQD.cf.actText);
//#                           publishEvent.publishMood(midlet.BombusQD.cf.moodText,midlet.BombusQD.cf.moodName);
//#                        }
//#                        public void no() { }
//#                     }; 
//#                     */
...
//#                        int size=moodName.getChildBlocks().size();
//#                        for(int i=0;i<size;i++){  
//#                            child =(JabberDataBlock)moodName.getChildBlocks().elementAt(i);
//#                            if (child.getTagName().equals("text")) continue;
//#                            moodIndex=Moods.getInstance().getMoodIngex(child.getTagName());
//#                        }
//#                     } catch (Exception ex) {
...
//#                       if (moodName.getChildBlockText("text")!=null){
//#                          if (moodName.getChildBlockText("text").length()>0) {
//#                          result.append("(")
//#                                  .append(moodName.getChildBlockText("text"))
//#                                  .append(")");
//#                           }
//#                         }
//#                       
...
//#                          c_mood.pepMoodName=Moods.getInstance().getMoodLabel(moodIndex);
//#                          c_mood.pepMoodText=moodName.getChildBlockText("text");                   
//#                     }
//#                     moodName=null;
//#                    }
//#              */
...
//#                         if (juick==null) {
//#                             juick=new Contact("#"+jid, "juick@juick.com/Juick" , Presence.PRESENCE_ONLINE, "both");
//#                             addContact(juick);
//#                         }
...
//#ifndef WMUC
            JabberDataBlock xmuc=pr.findNamespace("x", "http://jabber.org/protocol/muc#user");
                ...
                if (xmuc!=null) {//MUC only
                  ...
                  if(midlet.BombusQD.cf.dont_loadMC){
                     ///////.....
                  }
                  else
                  {
                    ...
                    try {
                        ...
                        Contact room = null;
                        if(rp!=-1)
                            room = getContact(from.substring(0, rp), false);
...
//#endif
                        ...
                        if (midlet.BombusQD.cf.storeConfPresence) {
                            ...
                            messageStore(room, chatPresence);
                            name=null;
                            chatPresence=null;
                        }
                        ...
                        if (ti==Presence.PRESENCE_ERROR) {
                           ...
                           StringBuffer sb = new StringBuffer(0);
                             sb.append(SR.MS_ERROR_).append(XmppError.findInStanza(pr).toString());
                             messageStore(room, new Msg(Msg.MESSAGE_TYPE_ERROR, name, null, sb.toString()));
...
//#                     c = getMetaContact(from,priority,ti);
//#endif                    
                    Msg m=new Msg( (ti==Presence.PRESENCE_AUTH ||
                         ti==Presence.PRESENCE_AUTH_ASK)?Msg.MESSAGE_TYPE_AUTH : Msg.MESSAGE_TYPE_PRESENCE, from, null, Prtext );                    
...
//#if METACONTACTS
//#                         if(c==null) c=getContact(from, enNIL);
//#else
        ...
        if (cont!=null){
            ...
            for(k=0;k<size;k++){
                ...
                if (i.getTagName().equals("item")) {
                    ...
                    updateContact(i.getAttribute("name"),i.getAttribute("jid"),group,i.getAttribute("subscription"), ask);
                    group=null;
                }
                i=null;
            }
...
//#endif
    ...
    public void beginConversation() { //todo: verify xmpp version
        if (theStream.isXmppV1())
            new SASLAuth(midlet.BombusQD.sd.account, this, theStream)
//#if SASL_XGOOGLETOKEN
//#              .setToken(token)
//#endif
             ;
...
//#         Vector childBlocks = new Vector(0);
//#         childBlocks = metaBlocks.getChildBlocks();
//#           int size = metaBlocks.getChildBlocks().size();   
//#           for(int i=0;i<size;i++){
//#                JabberDataBlock metaElement = (JabberDataBlock)childBlocks.elementAt(i);
//#                int hsize = hContacts.size();
//#                    Jid J=new Jid(metaElement.getAttribute("jid"));
//#                    Contact metaContact = findContact(J,false); // search by bare jid
//#                    if(metaContact!=null){
//#                       metaContact.metaContact = true;
//#                       metaContact.contactId = "id" + metaElement.getAttribute("tag");
//#                   }
//#            }
//#          reEnumRoster();
//#          sortContacts(null);
...
//#             if(find||msize==0) break;
//#             Contact search = (Contact)searchMeta.elementAt(y);
//#             for(int i=0;i<jid_size;i++) {
//#                    String element = (String)searchMetaContact.elementAt(i);
//#                    if(element.equals(from)){
//#                      int sizeIn = search.metaContacts.size();
//#                      for(int k=0;k<sizeIn;k++){
//#                          if(find) break;
//#                          Contact metaIn = (Contact)search.metaContacts.elementAt(k);
//#                          if(metaIn.bareJid.indexOf(from)>-1){
//#                              metaIn.group = search.group;//найденный контакт наследует группу
//#                              if(priority!=-1 && status!=-1) { //incoming presence
...
//#     
//#     
//#     private Vector searchMetaContact = new Vector();//all included jids
//#     private Vector searchMeta = new Vector();//all metaGroups
//#     
//#     
//#     private void convertContacts(){
//#         Contact c = null;
//#         int size = hContacts.size();
//#         Vector tempMetaContact = new Vector(0);
//#         synchronized (hContacts) {
//#           int i = 0;
//#            for(i=0;i<hContacts.size();i++) {
//#                 c =(Contact)hContacts.elementAt(i);
//#                 if(c.metaContact){
//#                   if(c.contactId.length()>0)
//#                   {
//#                     Contact metaContact = new Contact("", "metacontact", Presence.PRESENCE_CHAT, "both");
//#                     metaContact.setStatus(Presence.PRESENCE_CHAT);
//#                     //metaContact.setStatus("<metacontact>");
//#                     metaContact.setGroup(c.group);
//#                     metaContact.metaContact = true;
//#                     metaContact.contactId = "";
//#                       int foundContacts = 0;
//#                       tempMetaContact = new Vector(0);
//#                       for(int y=0;y<hContacts.size();y++){
//#                         Contact find =(Contact)hContacts.elementAt(y);
//#                         if(c.contactId.equals(find.contactId)) {
//#                             find.jid.setJid(find.bareJid);
//#                             tempMetaContact.addElement(find);
//#                             searchMetaContact.addElement(find.bareJid);
//#                             hContacts.removeElement(find);
//#                             foundContacts++;
//#                             y = -1;
//#                         }
//#                       }
//#                     metaContact.metaContacts = tempMetaContact;
//#                     metaContact.status = 0;
//#                     searchMeta.addElement(metaContact);//for fast search
//#                     hContacts.insertElementAt(metaContact,i);
//#                     i = -1;
//#                   }
//#                 }
//#           }
...
//#        synchronized (hContacts) {
//#           for(int y=0; y<hContacts.size(); y++) {
//#             Contact c = (Contact)hContacts.elementAt(y);
//#             eventMetaContact(c,true);//only collapse
//#           }
//#        }
//#       reEnumRoster();
//#     }   
...
//#       if(opened) c.contactId = "";
//#       if(c.metaContact && c.contactId.length()==0){
//#           int pos = hContacts.indexOf(c);
//#           if (pos>=0) {
    ...
    public void eventOk(){
       super.eventOk();
        ...
        if (e instanceof Contact) {
            Contact c = (Contact)e;
//#if METACONTACTS
//#             if(eventMetaContact(c,false)) return;
//#endif            
              if(midlet.BombusQD.cf.useClassicChat){
                new SimpleItemChat(display,this,c);
              } else{
...
//#endif
            ...
            case KEY_NUM0:
                ...
                synchronized(hContacts) {
                      for(int i=0;i<size;i++){
                        cc =(Contact)hContacts.elementAt(i);
                        cc.setIncoming(Contact.INC_NONE);
                        cc=null;                          
                      }
                }
                redraw();
...
//#             }
//#endif
            new SplashScreen(display, getMainBarItem(), midlet.BombusQD.cf.keyLock);
            return;
...
//#ifdef POPUPS
    public void showInfo() {
        Object fucused = getFocusedObject();        
        if (fucused==null) return;

        try {
            VirtualList.popup.next();
            if (fucused instanceof Group
//#ifndef WMUC
                    || fucused instanceof ConferenceGroup
//#endif
                    ) {
                fucused = null;
                return;
            }
            setWobbler(1, (Contact) null, null,fucused);
            fucused = null;
        } 
    ...
    public void setWobbler(int type, Contact contact, String info,Object focused) {
        if (info==null) {
            boolean isContact=(focused instanceof Contact);
            if(focused==null) focused = getFocusedObject();
...
//#endif
                mess.append("jid: ")
                    .append("\nMetaContact: "+cntact.metaContact)
                    .append("\nMetaID:"+cntact.contactId+"\nStatus "+cntact.status+"\n")
...
//#     }
//#     
//#     public String touchLeftCommand(){ return (midlet.BombusQD.cf.oldSE)?SR.MS_ITEM_ACTIONS:SR.MS_MENU; }
//#     public String touchRightCommand(){ return (midlet.BombusQD.cf.oldSE)?SR.MS_MENU:SR.MS_ITEM_ACTIONS; }
//#     
//#else
...
//#endif
    ...
    private static StringBuffer onl=new StringBuffer(0);
	...
	Object desiredFocus;
        ...
        public void queueEnum() {
		    ...
		    desiredFocus=null;
               ...
               synchronized (hContacts) {
                   for(int i=0;i<size;i++){
                      kk = (Contact)hContacts.elementAt(i);
                      if(kk.metaContact) {
                        if(kk.contactId.startsWith("opened") || kk.contactId.length()==0){
                          int sizeM = kk.metaContacts.size();
                          int online = 0;
                          String nick = null;
		      ...
		      grp.addInGroup(kk);
               ...
               selfContactGroup.visible=(midlet.BombusQD.cf.selfContact || selfContactGroup.tonlines>1 || selfContactGroup.unreadMessages>0 );
               
               // hiddens
               groups.getGroup(Groups.TYPE_IGNORE).visible= midlet.BombusQD.cf.ignore ;
               
               // transports
               Group transpGroup=groups.getGroup(Groups.TYPE_TRANSP);
               transpGroup.visible= (midlet.BombusQD.cf.showTransports || transpGroup.unreadMessages>0);
